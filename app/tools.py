...
class DB_Tools:
    ...
    def host_query(self):
        hosts = (
            self.db.session.query(CreateForm.hostname)
    ...
    def unit_query(self):
        units = (
            self.db.session.query(CreateUnits.unit_name)
    ...
    def host_unit_query(self):
        return self.db.session.query(CreateForm.hostname, CreateForm.unit_belong).all()

    def extra_ip_query(self):
        return self.db.session.query(CreateExIP).order_by(CreateExIP.id)
    ...
    def admin_user(self):
        try:
            admin = Users(id=0, email="admin@admin", password="admin", name="Admin")
    ...
    def session_query(self):
        return db.session.query(
            ActiveSessions.id, ActiveSessions.uuid, ActiveSessions.date_created
        ).all()
...
class Tools:
    ...
    def extract_csv_exip(self, content, forms):
        ...
        for data, form in zip(content, forms):
            exip = data[4].split("\n")
            if exip != [""]:
                ne = [CreateExIP(forms_id=form.id, extra_ip=ip) for ip in exip]
                new_exips.extend(ne)
        return new_exips
